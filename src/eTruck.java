...
public class eTruck implements Vehicle {
	private double fuelCapacity=12.0;
	private double mpu=45.0;
	private double currentFuel;
	private double miles;
	private String name="ELECTRIC TRUCK";
	...
	{   this.miles=miles;
		...
		if((currentFuel-(this.miles/mpu))<=0.0) 
		{
			...
			currentFuel=0;
			System.out.println("We are refilling  a full battery in "+this.name+" the cost is $"+this.refuel(fuelCapacity));
		}
	...
	{
		System.out.println("We are refueling"+this.name+"battery");
		if((fuelUnits>=this.getEmptyUnits()))
		{
			...
			return gasTotal*EFactory.ppu;
		 
		}
		else
		{
			currentFuel+=fuelUnits;
			return fuelUnits*EFactory.ppu;
		}
